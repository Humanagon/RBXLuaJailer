...
local function on_lua_error(failed, err, name)
	local src = failed.source
	local line = failed.lines[failed.pc - 1] or 0

	error(string.format('%s:%i: %s', name or src, line, err), 0)
end
...
function lua_wrap_state(proto, env, upval)
	local function wrapped(...)
		...
		if result[1] then
			...
			on_lua_error(failed, result[2], typeof(env.script) == "userdata" and getmetatable(env.script) == "SEMWrappedInstance1033983" and env.script:IsA("LuaSourceContainer") and env.script:GetFullName())
