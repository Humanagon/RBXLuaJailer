...
function luaZ:make_getF(source)
	...
	return function() -- chunk reader anonymous function here
		local buff = string_sub(source, pos, pos + LUAL_BUFFERSIZE - 1)
		pos = math_min(#source + 1, pos + LUAL_BUFFERSIZE)
		return buff
	end
end
...
------------------------------------------------------------------------
function WriteData(z, data)
	buffer_writestring(z.data, 0, data or "")
	z.dsize = data and #data or 0
end
...
function luaZ:init(reader, data)
	...
	local z = {}
	z.reader = reader
	z.data = buffer_create(luaX.MAX_DATA)
	WriteData(z, data)
	z.name = name
...
function luaZ:zgetc(z)
	...
	if n > 0 then
		z.n, z.p = n - 1, p
		return buffer_readstring(z.data, p - 1, 1)
	else
		return self:fill(z)
	end
end
...
luaX.MAX_SIZET = 16384 --4294967293
luaX.MAX_DATA = 4096
...
function luaX:init()
	local tokens, enums = {}, {}
	for v in string_gmatch(self.RESERVED, "[^\n]+") do
		local _, _, tok, str = string_find(v, "(%S+)%s+(%S+)")
		tokens[tok] = str
		enums[str] = tok
	end
	self.tokens = tokens
...
function luaX:chunkid(source, bufflen)
	local out
	local first = string_byte(source, 1)
	if first == 61 then
		out = string_sub(source, 2, bufflen)  -- remove first char
...
function luaX:token2str(ls, token)
	if string_sub(token, 1, 3) ~= "TK_" then
		if string_find(token, "%c") then
			return string_format("char(%d)", string_byte(token))
		end
		return token
	else
...
function luaX:lexerror(ls, msg, token)
	local function txtToken(ls, token)
		if token == "TK_NAME" or
			...
			return buffer_readstring(ls.buff, ls.bsize)
		else
			return self:token2str(ls, token)
		end
	end
	local buff = self:chunkid(ls.source, self.MAXSRC)
	local msg = string_format("%s:%d: %s", buff, ls.linenumber, msg)
	if token then
		msg = string_format("%s near "..self.LUA_QS, msg, txtToken(ls, token))
	end
...
function luaX:check_next(ls, set)
	if not string_find(set, ls.current, 1, 1) then
		return false
	end
...
function luaX:save(ls, c)
	...
	--end
	--ls.buff = ls.buff..c
	buffer_writestring(ls.buff, ls.bsize, c, 1)
	ls.bsize = ls.bsize + 1
end
...
function luaX:buffreplace(ls, from, to)
	...
	ls.buff = result
	--]]
	buffer_writestring(ls.buff, 0, string_gsub(buffer_readstring(ls.buff, 0, ls.bsize), string_gsub(from, "([^%w])", "%%%1"), to))
end
...
function luaX:trydecpoint(ls, Token)
	...
	local seminfo = self:str2d(buffer_readstring(ls.buff, 0, ls.bsize))
...
function luaX:read_numeral(ls, Token)
	-- lua_assert(string_find(ls.current, "%d"))
	repeat
		self:save_and_next(ls)
	until string_find(ls.current, "%D") and ls.current ~= "."
	if self:check_next(ls, "Ee") then  -- 'E'?
		self:check_next(ls, "+-")  -- optional exponent sign
	end
	while string_find(ls.current, "^%w$") or ls.current == "_" do
		self:save_and_next(ls)
	end
...
function luaX:read_long_string(ls, Token, sep)
	...
	while true do
		...
		elseif self:currIsNewline(ls) then
			...
			if not Token then ls.bsize = 0 end -- avoid wasting space
		else  -- default
			if Token then
				self:save_and_next(ls)
			else
	...
	if Token then
		local p = 3 + sep
		Token.seminfo = string_sub(buffer_readstring(ls.buff, 0, ls.bsize), p, -p)
	end
end
...
function luaX:read_string(ls, del, Token)
	...
	while ls.current ~= del do
		...
		elseif c == "\\" then
			...
			elseif c ~= "EOZ" then -- will raise an error next loop
				...
				local i = string_find("abfnrtv", c, 1, 1)
				if i then
					self:save(ls, string_sub("\a\b\f\n\r\t\v", i, i))
					self:nextc(ls)
	...
	Token.seminfo = string_sub(buffer_readstring(ls.buff, 0, ls.bsize), 2, -2)
end
	...
	while true do
		...
		elseif c == "-" then
			...
			if self:nextc(ls) == '[' then
				...
				ls.bsize = 0  -- 'skip_sep' may dirty the buffer
			end
			if sep >= 0 then
		...
		elseif c == "EOZ" then
			...
			elseif string_find(c, "[_%a]") then
				...
				repeat
					c = self:save_and_next(ls)
				until c == "EOZ" or not string_find(c, "[_%w]")
				local ts = buffer_readstring(ls.buff, 0, ls.bsize)
...
luaP.MAXARG_Bx  = math_ldexp(1, luaP.SIZE_Bx) - 1
luaP.MAXARG_sBx = math_floor(luaP.MAXARG_Bx / 2)  -- 'sBx' is signed

luaP.MAXARG_A = math_ldexp(1, luaP.SIZE_A) - 1
luaP.MAXARG_B = math_ldexp(1, luaP.SIZE_B) - 1
luaP.MAXARG_C = math_ldexp(1, luaP.SIZE_C) - 1
...
function luaP:DecodeInst(x)
	local i = {}
	local I = string_byte(x, 1)
	local op = I % 64
	i.OP = op
	I = string_byte(x, 2) * 4 + (I - op) / 64  -- 2 bits of c0 left
	local a = I % 256
	i.A = a
	I = string_byte(x, 3) * 4 + (I - a) / 256  -- 2 bits of c1 left
	local c = I % 512
	i.C = c
	i.B = string_byte(x, 4) * 2 + (I - c) / 512 -- 1 bits of c2 left
	local opmode = self.OpMode[tonumber(string_sub(self.opmodes[op + 1], 7, 7))]
	if opmode ~= "iABC" then
		i.Bx = i.B * 512 + i.C
	end
	return i
...
luaP.BITRK = math_ldexp(1, luaP.SIZE_B - 1)
...
function luaP:getBMode(m)
	return math_floor(self.opmodes[self.OpCode[m]] / 16) % 4
end

function luaP:getCMode(m)
	return math_floor(self.opmodes[self.OpCode[m]] / 4) % 4
end

function luaP:testAMode(m)
	return math_floor(self.opmodes[self.OpCode[m]] / 64) % 2
end

function luaP:testTMode(m)
	return math_floor(self.opmodes[self.OpCode[m]] / 128)
end
...
function luaU:make_setS()
	local buff = {}
	buff.data = ""
	buff.buff = buffer_create(luaX.MAX_DATA)
	buff.dsize = 0
	local writer = function(s, buff)  -- chunk writer
		...
		local len = #s
		while true do
			if luaX.MAX_DATA >= buff.dsize + len then
				buffer_writestring(buff.buff, buff.dsize, s)
				buff.dsize = buff.dsize + len
				break
			elseif buff.dsize ~= 0 then
				buff.data = buff.data .. buffer_readstring(buff.buff, 0, buff.dsize)
				buff.dsize = 0
			elseif len > 0 then
				buff.data = buff.data .. string_sub(s, 1, luaX.MAX_DATA)
				s = string_sub(s, luaX.MAX_DATA)
				len = #s
			else
				break
			end
		end
		-- print (#buff.data, #s, string_byte(s,1,1), s)
		return 0
	end
	return writer, buff
end
...
function luaU:from_int(x)
	...
	for i = 1, 4 do
		local c = x % 256
		v = v..string_char(c); x = math_floor(x / 256)
	end
	return v
end
...
function luaU:DumpChar(y, D)
	self:DumpBlock(string_char(y), D)
end
...
function luaK:fixjump(fs, pc, dest)
	...
	if math_abs(offset) > luaP.MAXARG_sBx then
...
function luaK:setlist(fs, base, nelems, tostore)
	local c = math_floor((nelems - 1)/luaP.LFIELDS_PER_FLUSH) + 1
...
function luaY:error_expected(ls, token)
	luaX:syntaxerror(ls,
		string_format(self.LUA_QS.." expected", luaX:token2str(ls, token)))
end
...
function luaY:errorlimit(fs, limit, what)
	local msg = (fs.f.linedefined == 0) and
		string_format("main function has more than %d %s", limit, what) or
		string_format("function at line %d has more than %d %s",
...
function luaY:check_match(ls, what, who, where)
	if not self:testnext(ls, what) then
		if where == ls.linenumber then
			...
			luaX:syntaxerror(ls, string_format(
...
function luaY:parser(L, z, buff, name)
	...
	L.nCcalls = 0
	lexstate.bsize = buff and #buff or 0
	lexstate.buff = buffer_create(luaX.MAX_SIZET)
...
return function (source, name)
	...
	return buff.data .. buffer_readstring(buff.buff, 0, buff.dsize)
end
